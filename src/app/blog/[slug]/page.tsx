...
export default async function BlogPostPage({
  ...
  const allPostsChrono = [...allPostsDesc].sort(
    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
  );
  const currentIndex = allPostsChrono.findIndex((p) => p.slug === slug);
  const previousPost =
    currentIndex > 0 ? allPostsChrono[currentIndex - 1] : null; // older
  const nextPost =
    currentIndex < allPostsChrono.length - 1
      ? allPostsChrono[currentIndex + 1]
      : null; // newer
